...
#define VTPC_DEFAULT_BLOCK_COUNT 128
...
static void drop_kernel_cache(int fd, off_t offset, size_t length) {
#if defined(POSIX_FADV_DONTNEED)
    (void)posix_fadvise(fd, offset, (off_t)length, POSIX_FADV_DONTNEED);
#else
    (void)fd;
    (void)offset;
    (void)length;
...
static void cache_init(void) {
    ...
    g_cache_capacity = parse_env_size("VTPC_CACHE_BLOCKS", VTPC_DEFAULT_BLOCK_COUNT);
    if (g_block_size == 0) {
        g_block_size = VTPC_DEFAULT_BLOCK_SIZE;
    }
    if (g_cache_capacity == 0) {
        g_cache_capacity = VTPC_DEFAULT_BLOCK_COUNT;
    }
    ...
    g_lru_head = &g_blocks[0];
    g_lru_tail = &g_blocks[g_cache_capacity - 1];
}
...
static int register_file(struct vtpc_file *file) {
    ...
    size_t old_capacity = g_file_capacity;
    size_t new_capacity = old_capacity ? old_capacity * 2 : 16;
    struct vtpc_file **new_table = realloc(g_files, new_capacity * sizeof(*new_table));
    if (!new_table) {
        errno = EMFILE;
        return -1;
    }
...
static int flush_block(struct cache_block *block) {
    ...
    off_t base_offset = block->block_index * (off_t)g_block_size;
    size_t length = end - start;
    size_t remaining = length;
    size_t local_offset = start;
    while (remaining > 0) {
        ssize_t written = pwrite(block->owner->os_fd,
    ...
    drop_kernel_cache(block->owner->os_fd, base_offset + (off_t)start, length);
...
static struct cache_block *find_block(struct vtpc_file *file, off_t block_index) {
    for (size_t i = 0; i < g_cache_capacity; ++i) {
...
static int load_block(struct cache_block *block) {
    ...
    if (block->owner->size > base_offset) {
        off_t remaining = block->owner->size - base_offset;
        expected = remaining >= (off_t)g_block_size ? g_block_size : (size_t)remaining;
    }

    ssize_t rd = 0;
    if (expected > 0) {
        rd = pread(block->owner->os_fd, block->data, g_block_size, base_offset);
        if (rd < 0) {
            return -1;
        }
    ...
    drop_kernel_cache(block->owner->os_fd, base_offset, g_block_size);
    block->dirty = false;
    block->dirty_start = 0;
    block->dirty_end = 0;
    return 0;
...
static struct cache_block *acquire_block(struct vtpc_file *file, off_t block_index) {
...
ssize_t vtpc_read(int fd, void *buf, size_t count) {
    ...
    while (total < count) {
        ...
        off_t block_index = file->offset / (off_t)g_block_size;
        size_t block_offset = (size_t)(file->offset % (off_t)g_block_size);
...
ssize_t vtpc_write(int fd, const void *buf, size_t count) {
    ...
    while (total < count) {
        ...
        size_t tail = block_offset + chunk;
        if (!block->dirty) {
        ...
        block->dirty = true;
        size_t new_valid = block_offset + chunk;
        if (new_valid > block->valid_bytes) {
            block->valid_bytes = new_valid;
